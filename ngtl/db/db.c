...
void updateTimeToDB(struct appTimeStruct *appTime)
{
    ...
    update = BCON_NEW("$set", "{", "upInt", BCON_DOUBLE(appTime->totalSecondsElapsed), "upTime",
                      BCON_UTF8(appTime->time), "}");
    if (!mongoc_collection_update_one(collection[UP_TIME], query, update, NULL, NULL, &error)) {
        fprintf(stderr, "%s\n", error.message);
        goto fail;
    }
fail:
...
void updateAppStatsToDB(void)
{
    for (uint8_t portid = 0; portid < 2; portid++) {
        ...
        update = BCON_NEW(
                ...
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeFTP), "tcpTypeSSH",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeSSH), "tcpTypeTELNET",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeTELNET), "tcpTypeSMTP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeSMTP), "tcpTypeHTTP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeHTTP), "tcpTypeHTTPS",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeHTTPS), "tcpTypeIMAPS",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeIMAPS), "tcpTypeMSSQL",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeMSSQL), "tcpTypeORACLESQL",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeORACLESQL), "tcpTypePPTP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypePPTP), "tcpTypeMYSQL",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeMYSQL), "tcpTypeRDP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeRDP), "tcpTypeVNC",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeVNC), "tcpTypeNETBIOS",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeNETBIOS), "tcpTypeBGP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeBGP), "tcpTypeSIP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeSIP), "tcpTypeLDAP",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeLDAP), "tcpTypeSOCKS",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeSOCKS), "tcpTypeSQUID",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypeSQUID), "tcpTypePOSTGRESQL",
                BCON_DOUBLE(appStatsData[portid].tcpServicesData.tcpTypePOSTGRESQL), "tcpTypeREDIS",
                ...
                "udpTypeSNMP", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeSNMP),
                "udpTypeSYSLOG", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeSYSLOG),
                "udpTypeRIP", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeRIP),
                "udpTypeRADIUS", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeRADIUS),
                "udpTypeX11", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeX11),
                "udpTypeOPENVPN", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeOPENVPN),
                "udpTypeUNKNOWN", BCON_DOUBLE(appStatsData[portid].udpServicesData.udpTypeUNKNOWN),
                "}");

        if (!mongoc_collection_update_one(collection[PORT_STATS], query, update, NULL, NULL,
                                          &error)) {
            fprintf(stderr, "%s\n", error.message);
            goto fail;
        }
...
void updateInterfaceStatsToDB(struct interfaceStatsStruct *interfaceStatsData, uint8_t portid)
{
    ...
    update = BCON_NEW("$set", "{", "pktsReceived", BCON_DOUBLE(interfaceStatsData->pktsReceived),
                      "pktsSent", BCON_DOUBLE(interfaceStatsData->pktsSent), "pktsDropped",
    ...
    if (!mongoc_collection_update_one(collection[INTERFACE_STATS], query, update, NULL, NULL,
                                      &error)) {
        fprintf(stderr, "%s\n", error.message);
        goto fail;
    }
...
void updateDeviceStatsToDB(struct deviceStatsStruct *deviceStatsData)
{
    ...
    update = BCON_NEW("$set", "{", "totalPktsReceived",
    ...
    if (!mongoc_collection_update_one(collection[DEVICE_STATS], query, update, NULL, NULL,
                                      &error)) {
        fprintf(stderr, "%s\n", error.message);
        goto fail;
    }
...
void insertNetstatToDB(struct netstatStruct *netstatData)
{
    ...
    if (!mongoc_collection_insert_one(collection[NET_STATS], doc, NULL, NULL, &error))
