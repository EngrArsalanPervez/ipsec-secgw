...
cd examples/ipsec-secgw/
./build/ipsec-secgw -c 0xf -n 4 --socket-mem 1024,0 --vdev "crypto_openssl" -- -p 0x3 -P -u 0x2 --config="(0,0,0),(1,0,1)" -d L2 -f ./ngtl/config/IPE_L2.cfg --transfer-mode poll -t 1
until ipsec-secgw; do
  echo "Server 'ipsec-secgw' crashed with exit code $?.  Respawning.." >&2
  sleep 2
  ./build/ipsec-secgw -c 0xf -n 4 --socket-mem 1024,0 --vdev "crypto_openssl" -- -p 0x3 -P -u 0x2 --config="(0,0,0),(1,0,1)" -d L2 -f ./ngtl/config/IPE_L2.cfg --transfer-mode poll -t 1
