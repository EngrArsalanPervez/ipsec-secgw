...
void print_kni_stats(void)
{
    printf("\nrx_packets:: %35" PRIu64 "\ntx_packets:: %35" PRIu64 "\nrx_dropped:: %35" PRIu64
           ...
           kni_stats[0].rx_packets, kni_stats[0].tx_packets, kni_stats[0].rx_dropped,
           kni_stats[0].tx_dropped);
}
...
void kni_filter_ike_packets(int32_t nb_rx, struct rte_mbuf **pkts, uint16_t port_id,
...
int kni_change_mtu(uint16_t port_id, unsigned int new_mtu)
{
    ...
    ret = rte_eth_rx_queue_setup(port_id, 0, nb_rxd, rte_eth_dev_socket_id(port_id), &rxq_conf,
                                 pktmbuf_pool);
    if (ret < 0) {
...
int kni_alloc(uint16_t port_id)
{
    ...
    for (i = 0; i < params[port_id]->nb_kni; i++) {
        ...
        if (i == 0) {
            ...
            memcpy(hardcoded_mac.addr_bytes, &device_type.vEth0, RTE_ETHER_ADDR_LEN);
