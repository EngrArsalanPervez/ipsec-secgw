...
void *subscriber_thread(void *arg)
{
    ...
    if ((s = natsOptions_LoadCATrustedCertificates(opts, "/etc/nats/nats-certs/rootCA.pem")) !=
        NATS_OK)
        goto cleanup;

    // 3. Load the client's certificate and private key to present to the server
    if ((s = natsOptions_LoadCertificatesChain(opts, "/etc/nats/nats-certs/client-cert.pem",
    ...
    if ((s = natsConnection_Connect(&conn, opts)) != NATS_OK)
        goto cleanup;
    if ((s = natsConnection_Subscribe(&sub, conn, subject, onMsg, NULL)) != NATS_OK)
        goto cleanup;
