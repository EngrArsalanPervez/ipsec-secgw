...
static inline enum pkt_type process_ipsec_get_pkt_type(struct rte_mbuf *pkt, uint8_t **nlp)
{
    ...
    if (RTE_ETH_IS_IPV4_HDR(ptype)) {
        *nlp = RTE_PTR_ADD(eth, RTE_ETHER_HDR_LEN + offsetof(struct ip, ip_p));
        ...
        *nlp = RTE_PTR_ADD(eth, RTE_ETHER_HDR_LEN + offsetof(struct ip6_hdr, ip6_nxt));
        if ((ptype & RTE_PTYPE_TUNNEL_MASK) == RTE_PTYPE_TUNNEL_ESP)
...
static inline void ipsec_event_pre_forward(struct rte_mbuf *m, unsigned int port_id)
{
...
static inline void ev_vector_attr_update(struct rte_event_vector *vec, struct rte_mbuf *pkt)
{
    if (vec->port == 0xFFFF) {
        vec->port = pkt->port;
        return;
...
static inline void prepare_out_sessions_tbl(struct sa_ctx *sa_out, struct port_drv_mode_data *data,
                                            uint16_t size)
{
    ...
    for (i = 0; i < sa_out->nb_sa; i++) {
        ...
        if (sa->portid >= size) {
            RTE_LOG(ERR, IPSEC, "Port id >= than table size %d, %d\n", sa->portid, size);
            continue;
        }
...
static inline int check_sp(struct sp_ctx *sp, const uint8_t *nlp, uint32_t *sa_idx)
{
    uint32_t res;
    ...
    rte_acl_classify((struct rte_acl_ctx *)sp, &nlp, &res, 1, DEFAULT_MAX_CATEGORIES);
...
static inline uint16_t get_route(struct rte_mbuf *pkt, struct route_table *rt, enum pkt_type type)
{
...
static inline int process_ipsec_ev_inbound(struct ipsec_ctx *ctx, struct route_table *rt,
                                           struct rte_event *ev)
{
...
static inline int process_ipsec_ev_outbound(struct ipsec_ctx *ctx, struct route_table *rt,
                                            struct rte_event *ev)
{
    ...
    rte_security_set_pkt_metadata(sess->security.ctx, sess->security.ses, pkt, NULL);
...
static inline int ipsec_ev_route_pkts(struct rte_event_vector *vec, struct route_table *rt,
...
static inline int process_ipsec_ev_inbound_vector(struct ipsec_ctx *ctx, struct route_table *rt,
...
static inline int process_ipsec_ev_outbound_vector(struct ipsec_ctx *ctx, struct route_table *rt,
                                                   struct rte_event_vector *vec)
{
    struct ipsec_traffic t;
    struct rte_mbuf *pkt;
...
static inline int process_ipsec_ev_drv_mode_outbound_vector(struct rte_event_vector *vec,
                                                            struct port_drv_mode_data *data)
{
    ...
    for (i = 0; i < vec->nb_elem; i++) {
        ...
        rte_security_set_pkt_metadata(data[port_id].ctx, data[port_id].sess, pkt, NULL);
...
static inline void ipsec_ev_vector_process(struct lcore_conf_ev_tx_int_port_wrkr *lconf,
                                           struct eh_event_link_info *links, struct rte_event *ev)
{
    ...
    if (is_unprotected_port(pkt->port))
        ret = process_ipsec_ev_inbound_vector(&lconf->inbound, &lconf->rt, vec);
    else
        ret = process_ipsec_ev_outbound_vector(&lconf->outbound, &lconf->rt, vec);

    if (likely(ret > 0)) {
        vec->nb_elem = ret;
        rte_event_eth_tx_adapter_enqueue(links[0].eventdev_id, links[0].event_port_id, ev, 1, 0);
    } else {
        rte_mempool_put(rte_mempool_from_obj(vec), vec);
    }
}

static inline void ipsec_ev_vector_drv_mode_process(struct eh_event_link_info *links,
                                                    struct rte_event *ev,
                                                    struct port_drv_mode_data *data)
{
    struct rte_event_vector *vec = ev->vec;
    ...
    if (!is_unprotected_port(pkt->port))
        vec->nb_elem = process_ipsec_ev_drv_mode_outbound_vector(vec, data);
    if (vec->nb_elem > 0)
        rte_event_eth_tx_adapter_enqueue(links[0].eventdev_id, links[0].event_port_id, ev, 1, 0);
    else
        rte_mempool_put(rte_mempool_from_obj(vec), vec);
 ...
 */
static void ipsec_wrkr_non_burst_int_port_drv_mode(struct eh_event_link_info *links,
                                                   uint8_t nb_links)
{
    ...
    if (nb_links != 1) {
        RTE_LOG(INFO, IPSEC, "Multiple links not supported. Using first link\n");
    }

    RTE_LOG(INFO, IPSEC, " -- lcoreid=%u event_port_id=%u\n", lcore_id, links[0].event_port_id);
    while (!force_quit) {
        /* Read packet from event queues */
        nb_rx = rte_event_dequeue_burst(links[0].eventdev_id, links[0].event_port_id,
                                        &ev, /* events */
                                        1, /* nb_events */
                                        0 /* timeout_ticks */);
 ...
 */
static void ipsec_wrkr_non_burst_int_port_app_mode(struct eh_event_link_info *links,
                                                   uint8_t nb_links)
{
    ...
    lconf.inbound.session_priv_pool = socket_ctx[socket_id].session_priv_pool;
    ...
    lconf.outbound.session_priv_pool = socket_ctx[socket_id].session_priv_pool;
    ...
    while (!force_quit) {
        ...
        if (is_unprotected_port(ev.mbuf->port))
            ret = process_ipsec_ev_inbound(&lconf.inbound, &lconf.rt, &ev);
        else
            ret = process_ipsec_ev_outbound(&lconf.outbound, &lconf.rt, &ev);
        if (ret != 1)
...
static uint8_t ipsec_eventmode_populate_wrkr_params(struct eh_app_worker_params *wrkrs)
{
...
{
    struct eh_app_worker_params ipsec_wrkr[IPSEC_EVENTMODE_WORKERS] = { { { { 0 } }, NULL } };
