...
const struct supported_cipher_algo cipher_algos[] = { { .keyword = "null",
...
const struct supported_auth_algo auth_algos[] = {
    { .keyword = "null",
      ...
      .key_len = 0,
      .key_not_req = 1 },
    { .keyword = "sha1-hmac", .algo = RTE_CRYPTO_AUTH_SHA1_HMAC, .digest_len = 12, .key_len = 20 },
    { .keyword = "sha256-hmac",
...
static const struct supported_cipher_algo *find_match_cipher_algo(const char *cipher_keyword)
{
    size_t i;

    for (i = 0; i < RTE_DIM(cipher_algos); i++) {
        const struct supported_cipher_algo *algo = &cipher_algos[i];
...
static const struct supported_auth_algo *find_match_auth_algo(const char *auth_keyword)
{
    size_t i;

    for (i = 0; i < RTE_DIM(auth_algos); i++) {
        const struct supported_auth_algo *algo = &auth_algos[i];
...
static const struct supported_aead_algo *find_match_aead_algo(const char *aead_keyword)
{
    size_t i;

    for (i = 0; i < RTE_DIM(aead_algos); i++) {
        const struct supported_aead_algo *algo = &aead_algos[i];
 ...
 *  0:   failed
 */
static uint32_t parse_key_string(const char *key_str, uint8_t *key)
{
...
static int extend_sa_arr(struct ipsec_sa **sa_tbl, uint32_t cur_cnt, uint32_t *cur_sz)
{
    if (*sa_tbl == NULL) {
    ...
    if (cur_cnt >= *cur_sz) {
        *sa_tbl = realloc(*sa_tbl, *cur_sz * sizeof(struct ipsec_sa) * 2);
        if (*sa_tbl == NULL)
            return -1;
        /* clean reallocated extra space */
        memset(&(*sa_tbl)[*cur_sz], 0, *cur_sz * sizeof(struct ipsec_sa));
        *cur_sz *= 2;
    }
...
void parse_sa_tokens(char **tokens, uint32_t n_tokens, struct parse_status *status)
{
    ...
    for (ti = 2; ti < n_tokens; ti++) {
        ...
        if (strcmp(tokens[ti], "cipher_algo") == 0) {
            ...
            APP_CHECK_PRESENCE(cipher_algo_p, tokens[ti], status);
            if (status->status < 0)
                return;
            ...
            APP_CHECK(strcmp(tokens[ti], "cipher_key") == 0, status,
            ...
            key_len = parse_key_string(tokens[ti], rule->cipher_key);
            APP_CHECK(key_len == rule->cipher_key_len, status, "unrecognized input \"%s\"",
                      tokens[ti]);
            if (status->status < 0)
                return;

            if (algo->algo == RTE_CRYPTO_CIPHER_AES_CBC || algo->algo == RTE_CRYPTO_CIPHER_3DES_CBC)
                rule->salt = (uint32_t)rte_rand();

            if (algo->algo == RTE_CRYPTO_CIPHER_AES_CTR) {
                key_len -= 4;
                rule->cipher_key_len = key_len;
                memcpy(&rule->salt, &rule->cipher_key[key_len], 4);
            }
        ...
        if (strcmp(tokens[ti], "auth_algo") == 0) {
            ...
            APP_CHECK_PRESENCE(auth_algo_p, tokens[ti], status);
            if (status->status < 0)
                return;
            ...
            APP_CHECK(strcmp(tokens[ti], "auth_key") == 0, status,
                      "unrecognized input \"%s\", "
                      "expect \"auth_key\"",
                      tokens[ti]);
            if (status->status < 0)
            ...
            key_len = parse_key_string(tokens[ti], rule->auth_key);
            APP_CHECK(key_len == rule->auth_key_len, status, "unrecognized input \"%s\"",
                      tokens[ti]);
            if (status->status < 0)
                return;
            ...
            if (algo->algo == RTE_CRYPTO_AUTH_AES_GMAC) {
                ...
                rule->iv_len = algo->iv_len;
                memcpy(&rule->salt, &rule->auth_key[key_len], 4);
            }
        ...
        if (strcmp(tokens[ti], "aead_algo") == 0) {
            ...
            APP_CHECK_PRESENCE(aead_algo_p, tokens[ti], status);
            if (status->status < 0)
                return;
            ...
            APP_CHECK(strcmp(tokens[ti], "aead_key") == 0, status,
                      "unrecognized input \"%s\", "
                      "expect \"aead_key\"",
                      tokens[ti]);
            if (status->status < 0)
        ...
        if (strcmp(tokens[ti], "src") == 0) {
            ...
            if (IS_IP4_TUNNEL(rule->flags)) {
                ...
                APP_CHECK(parse_ipv4_addr(tokens[ti], &ip, NULL) == 0, status,
                          "unrecognized input \"%s\", "
                          "expect valid ipv4 addr",
                          tokens[ti]);
                if (status->status < 0)
                    return;
                rule->src.ip.ip4 = rte_bswap32((uint32_t)ip.s_addr);
            } else if (IS_IP6_TUNNEL(rule->flags)) {
                struct in6_addr ip;

                APP_CHECK(parse_ipv6_addr(tokens[ti], &ip, NULL) == 0, status,
                          "unrecognized input \"%s\", "
                          "expect valid ipv6 addr",
                          tokens[ti]);
                if (status->status < 0)
                    return;
                memcpy(rule->src.ip.ip6.ip6_b, ip.s6_addr, 16);
        ...
        if (strcmp(tokens[ti], "dst") == 0) {
            ...
            if (IS_IP4_TUNNEL(rule->flags)) {
                ...
                if (status->status < 0)
                    return;
                rule->dst.ip.ip4 = rte_bswap32((uint32_t)ip.s_addr);
            } else if (IS_IP6_TUNNEL(rule->flags)) {
                struct in6_addr ip;
        ...
        if (strcmp(tokens[ti], "type") == 0) {
            ...
            if (strcmp(tokens[ti], "inline-crypto-offload") == 0)
            ...
            else if (strcmp(tokens[ti], "inline-protocol-offload") == 0)
                ips->type = RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL;
            else if (strcmp(tokens[ti], "lookaside-protocol-offload") == 0)
                ...
                APP_CHECK(0, status, "Invalid input \"%s\"", tokens[ti]);
                return;
            }
        ...
        if (strcmp(tokens[ti], "port_id") == 0) {
            ...
            if (rule->portid == UINT16_MAX)
                ...
                APP_CHECK(0, status, "portid %s not matching with already assigned portid %u",
        ...
        if (strcmp(tokens[ti], "fallback") == 0) {
            ...
            APP_CHECK(app_sa_prm.enable, status,
                      "Fallback session "
                      "not allowed for legacy mode.");
            if (status->status < 0)
                return;
            APP_CHECK(ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO, status,
                      "Fallback session allowed if primary session "
                      "is of type inline-crypto-offload only.");
            if (status->status < 0)
                return;
            APP_CHECK(rule->direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS, status,
        ...
        if (strcmp(tokens[ti], "flow-direction") == 0) {
            ...
            case RTE_SECURITY_ACTION_TYPE_CPU_CRYPTO:
                ...
                status_p = check_flow_params(rule->portid, rule->fdir_qid);
                if (status_p < 0) {
            ...
            default:
                APP_CHECK(0, status, "flow director not supported for security session type %d",
                          ips->type);
                return;
            }
            continue;
    ...
    if (aead_algo_p) {
        APP_CHECK(cipher_algo_p == 0, status, "AEAD used, no need for cipher options");
        if (status->status < 0)
            return;

        APP_CHECK(auth_algo_p == 0, status, "AEAD used, no need for auth options");
        if (status->status < 0)
            return;
    } else {
    ...
    if (!type_p || (!portid_p && ips->type != RTE_SECURITY_ACTION_TYPE_CPU_CRYPTO)) {
        ips->type = RTE_SECURITY_ACTION_TYPE_NONE;
    }
...
static void print_one_sa_rule(const struct ipsec_sa *sa, int inbound)
{
    uint32_t i;
    uint8_t a, b, c, d;
...
static struct sa_ctx *sa_create(const char *name, int32_t socket_id, uint32_t nb_sa)
{
    char s[PATH_MAX];
    ...
    mz = rte_memzone_reserve(s, mz_size, socket_id, RTE_MEMZONE_1GB | RTE_MEMZONE_SIZE_HINT_ONLY);
    if (mz == NULL) {
    ...
    sa_ctx = rte_zmalloc(NULL, sizeof(struct sa_ctx) + sizeof(struct ipsec_sa) * nb_sa,
                         RTE_CACHE_LINE_SIZE);
...
static int check_eth_dev_caps(uint16_t portid, uint32_t inbound, uint32_t tso)
{
    ...
    if (retval != 0) {
        RTE_LOG(ERR, IPSEC, "Error during getting device (port %u) info: %s\n", portid,
                strerror(-retval));
    ...
    if (inbound) {
        if ((dev_info.rx_offload_capa & RTE_ETH_RX_OFFLOAD_SECURITY) == 0) {
            RTE_LOG(WARNING, PORT, "hardware RX IPSec offload is not supported\n");
            return -EINVAL;
        }

    } else { /* outbound */
        if ((dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_SECURITY) == 0) {
            RTE_LOG(WARNING, PORT, "hardware TX IPSec offload is not supported\n");
            return -EINVAL;
        }
        if (tso && (dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_TCP_TSO) == 0) {
            RTE_LOG(WARNING, PORT, "hardware TCP TSO offload is not supported\n");
            return -EINVAL;
        }
    }
    return 0;
 ...
 */
static int get_spi_proto(uint32_t spi, enum rte_security_ipsec_sa_direction dir,
                         struct ip_addr ip_addr[2], uint32_t mask[2])
{
    int32_t rc4, rc6;

    rc4 = sp4_spi_present(spi, dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS, ip_addr, mask);
    rc6 = sp6_spi_present(spi, dir == RTE_SECURITY_IPSEC_SA_DIR_INGRESS, ip_addr, mask);
    ...
    if (rc4 >= 0) {
        ...
        RTE_LOG(ERR, IPSEC, "%s: SPI %u is not used by any SP rule\n", __func__, spi);
        return -EINVAL;
    } else
        return IPPROTO_IPV6;
}
 ...
 */
static int sa_add_address_inline_crypto(struct ipsec_sa *sa)
{
    int protocol;
    ...
    if (protocol < 0)
        ...
        if (mask[0] == IP4_FULL_MASK && mask[1] == IP4_FULL_MASK && ip_addr[0].ip.ip4 != 0 &&
            ip_addr[1].ip.ip4 != 0) {
        ...
        if (mask[0] == IP6_FULL_MASK && mask[1] == IP6_FULL_MASK &&
            (ip_addr[0].ip.ip6.ip6[0] != 0 || ip_addr[0].ip.ip6.ip6[1] != 0) &&
            (ip_addr[1].ip.ip6.ip6[0] != 0 || ip_addr[1].ip.ip6.ip6[1] != 0)) {
...
static int sa_add_rules(struct sa_ctx *sa_ctx, const struct ipsec_sa entries[], uint32_t nb_entries,
                        uint32_t inbound, struct socket_ctx *skt_ctx)
{
    struct ipsec_sa *sa;
    uint32_t i, idx;
        ...
        *sa = entries[i];
        ...
        case TRANSPORT:
            if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO) {
                inline_status = sa_add_address_inline_crypto(sa);
                if (inline_status < 0)
        ...
        if (sa->aead_algo == RTE_CRYPTO_AEAD_AES_GCM || sa->aead_algo == RTE_CRYPTO_AEAD_AES_CCM ||
            ...
            sa_ctx->xf[idx].a.aead.key.length = sa->cipher_key_len;
            sa_ctx->xf[idx].a.aead.op = (inbound == 1) ? RTE_CRYPTO_AEAD_OP_DECRYPT :
                                                         RTE_CRYPTO_AEAD_OP_ENCRYPT;
            sa_ctx->xf[idx].a.next = NULL;
            sa_ctx->xf[idx].a.aead.iv.offset = IV_OFFSET;
            sa_ctx->xf[idx].a.aead.iv.length = iv_length;
            sa_ctx->xf[idx].a.aead.aad_length = sa->aad_len + aad_length;
            sa_ctx->xf[idx].a.aead.digest_length = sa->digest_len;
            ...
            if (inbound) {
                ...
                sa_ctx->xf[idx].b.cipher.key.length = sa->cipher_key_len;
                sa_ctx->xf[idx].b.cipher.op = RTE_CRYPTO_CIPHER_OP_DECRYPT;
                ...
                sa_ctx->xf[idx].a.auth.key.length = sa->auth_key_len;
                sa_ctx->xf[idx].a.auth.digest_length = sa->digest_len;
                sa_ctx->xf[idx].a.auth.op = RTE_CRYPTO_AUTH_OP_VERIFY;
                ...
                sa_ctx->xf[idx].a.cipher.key.length = sa->cipher_key_len;
                sa_ctx->xf[idx].a.cipher.op = RTE_CRYPTO_CIPHER_OP_ENCRYPT;
                sa_ctx->xf[idx].a.next = NULL;
                ...
                sa_ctx->xf[idx].b.auth.key.length = sa->auth_key_len;
                sa_ctx->xf[idx].b.auth.digest_length = sa->digest_len;
                sa_ctx->xf[idx].b.auth.op = RTE_CRYPTO_AUTH_OP_GENERATE;
                sa_ctx->xf[idx].b.auth.iv.offset = IV_OFFSET;
                sa_ctx->xf[idx].b.auth.iv.length = auth_iv_length;
            }

            if (sa->auth_algo == RTE_CRYPTO_AUTH_AES_GMAC) {
                sa->xforms = inbound ? &sa_ctx->xf[idx].a : &sa_ctx->xf[idx].b;
                sa->xforms->next = NULL;
        ...
        if (ips->type == RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL ||
            ...
            if (rc != 0) {
                RTE_LOG(ERR, IPSEC_ESP, "create_inline_session() failed\n");
                return -EINVAL;
            }
        }
        ...
        if (sa->fdir_flag && inbound) {
            ...
            if (rc != 0)
                RTE_LOG(ERR, IPSEC_ESP, "create_ipsec_esp_flow() failed\n");
        }
...
static inline int sa_out_add_rules(struct sa_ctx *sa_ctx, const struct ipsec_sa entries[],
                                   uint32_t nb_entries, struct socket_ctx *skt_ctx)
{
    return sa_add_rules(sa_ctx, entries, nb_entries, 0, skt_ctx);
}

static inline int sa_in_add_rules(struct sa_ctx *sa_ctx, const struct ipsec_sa entries[],
                                  uint32_t nb_entries, struct socket_ctx *skt_ctx)
{
 ...
 */
static void fill_ipsec_app_sa_prm(struct rte_ipsec_sa_prm *prm, const struct app_sa_prm *app_prm)
{
...
static int fill_ipsec_sa_prm(struct rte_ipsec_sa_prm *prm, const struct ipsec_sa *ss,
                             const struct rte_ipv4_hdr *v4, struct rte_ipv6_hdr *v6)
{
    ...
    prm->ipsec_xform.mode = (IS_TRANSPORT(ss->flags)) ? RTE_SECURITY_IPSEC_SA_MODE_TRANSPORT :
...
static int fill_ipsec_session(struct rte_ipsec_session *ss, struct rte_ipsec_sa *sa)
{
    int32_t rc = 0;
 ...
 */
static int ipsec_sa_init(struct ipsec_sa *lsa, struct rte_ipsec_sa *sa, uint32_t sa_size)
{
    int rc;
    struct rte_ipsec_sa_prm prm;
    struct rte_ipsec_session *ips;
    struct rte_ipv4_hdr v4 = {
        .version_ihl = IPVERSION << 4 | sizeof(v4) / RTE_IPV4_IHL_MULTIPLIER,
 ...
 * one per session.
 */
static int ipsec_satbl_init(struct sa_ctx *ctx, uint32_t nb_ent, int32_t socket)
{
    int32_t rc, sz;
    uint32_t i, idx;
    size_t tsz;
    ...
    if (ctx->satbl == NULL) {
        RTE_LOG(ERR, IPSEC, "%s(%p, %u, %d): failed to allocate %zu bytes\n", __func__, ctx, nb_ent,
                socket, tsz);
        return -ENOMEM;
    }
...
static int sa_cmp(const void *p, const void *q)
{
 ...
 */
int sa_spi_present(struct sa_ctx *sa_ctx, uint32_t spi, int inbound)
{
    uint32_t num;
    struct ipsec_sa *sa;
...
void sa_init(struct socket_ctx *ctx, int32_t socket_id)
{
    int32_t rc;
    const char *name;
    ...
    if (nb_sa_in > 0) {
        ...
        rc = ipsec_sad_create(name, &ctx->sa_in->sad, socket_id, &sa_in_cnt);
        if (rc != 0)
        ...
        if (app_sa_prm.enable != 0) {
            rc = ipsec_satbl_init(ctx->sa_in, nb_sa_in, socket_id);
            if (rc != 0)
                rte_exit(EXIT_FAILURE, "failed to init inbound SAs\n");
        }
    } else
    ...
    if (nb_sa_out > 0) {
        ...
        if (app_sa_prm.enable != 0) {
            rc = ipsec_satbl_init(ctx->sa_out, nb_sa_out, socket_id);
            if (rc != 0)
                rte_exit(EXIT_FAILURE, "failed to init outbound SAs\n");
        }
    } else
...
int inbound_sa_check(struct sa_ctx *sa_ctx, struct rte_mbuf *m, uint32_t sa_idx)
{
...
void inbound_sa_lookup(struct sa_ctx *sa_ctx, struct rte_mbuf *pkts[], void *sa_arr[],
                       uint16_t nb_pkts)
{
    uint32_t i;
    void *result_sa;
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if (MBUF_NO_SEC_OFFLOAD(pkts[i]) && sa->fallback_sessions > 0) {
...
void outbound_sa_lookup(struct sa_ctx *sa_ctx, uint32_t sa_idx[], void *sa[], uint16_t nb_pkts)
{
    uint32_t i;
 ...
 */
int sa_check_offloads(uint16_t port_id, uint64_t *rx_offloads, uint64_t *tx_offloads)
{
    ...
    *tx_offloads = 0;
    ...
    for (idx_sa = 0; idx_sa < nb_sa_in; idx_sa++) {
        ...
        if ((rule_type == RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO ||
             rule_type == RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL) &&
            rule->portid == port_id)
        ...
        if (rule->portid == port_id) {
            ...
            case RTE_SECURITY_ACTION_TYPE_INLINE_PROTOCOL:
                ...
                *tx_offloads |= RTE_ETH_TX_OFFLOAD_SECURITY;
                if (rule->mss)
                    *tx_offloads |= (RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM);
                break;
            case RTE_SECURITY_ACTION_TYPE_INLINE_CRYPTO:
                *tx_offloads |= RTE_ETH_TX_OFFLOAD_SECURITY;
                if (rule->mss)
