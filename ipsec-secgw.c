...
#define ETHADDR_TO_UINT64(addr)                                                            \
    __BYTES_TO_UINT64((addr)->addr_bytes[0], (addr)->addr_bytes[1], (addr)->addr_bytes[2], \
                      (addr)->addr_bytes[3], (addr)->addr_bytes[4], (addr)->addr_bytes[5], 0, 0)
...
struct app_sa_prm app_sa_prm = { .enable = 0, .cache_sz = SA_CACHE_SZ, .udp_encap = 0 };
 ...
 */
static int multi_seg_required(void)
{
    return (MTU_TO_FRAMELEN(mtu_size) + RTE_PKTMBUF_HEADROOM > frame_buf_size || frag_tbl_sz != 0);
}

static inline void adjust_ipv4_pktlen(struct rte_mbuf *m, const struct rte_ipv4_hdr *iph,
                                      uint32_t l2_len)
{
    uint32_t plen, trim;
...
static inline void adjust_ipv6_pktlen(struct rte_mbuf *m, const struct rte_ipv6_hdr *iph,
                                      uint32_t l2_len)
{
    uint32_t plen, trim;
...
static inline void prepare_one_packet(struct rte_mbuf *pkt, struct ipsec_traffic *t)
{
    ...
    if (eth->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
        ...
        case IPPROTO_UDP:
            if (app_sa_prm.udp_encap == 1) {
                ip4_hdr_len =
                        ((iph4->version_ihl & RTE_IPV4_HDR_IHL_MASK) * RTE_IPV4_IHL_MULTIPLIER);
        ...
        while (next_proto != IPPROTO_ESP && l3len < pkt->data_len &&
               (next_proto = rte_ipv6_get_next_ext(p + l3len, next_proto, &ext_len)) >= 0)
            l3len += ext_len;
...
static inline void prepare_traffic(struct rte_mbuf **pkts, struct ipsec_traffic *t,
                                   uint16_t nb_pkts)
{
    int32_t i;
                                  ...
                                  const struct lcore_conf *qconf)
{
    ...
    memcpy(&ethhdr->src_addr, &ethaddr_tbl[port].src, sizeof(struct rte_ether_addr));
    memcpy(&ethhdr->dst_addr, &ethaddr_tbl[port].dst, sizeof(struct rte_ether_addr));
}

static inline void prepare_tx_burst(struct rte_mbuf *pkts[], uint16_t nb_pkts, uint16_t port,
                                    const struct lcore_conf *qconf)
{
    int32_t i;
...
static inline int32_t send_burst(struct lcore_conf *qconf, uint16_t n, uint16_t port)
{
 ...
 */
static inline uint32_t send_fragment_packet(struct lcore_conf *qconf, struct rte_mbuf *m,
                                            uint16_t port, uint8_t proto)
{
    ...
    if (proto == IPPROTO_IP)
        rc = rte_ipv4_fragment_packet(m, tbl->m_table + len, n, mtu_size, qconf->frag.pool_dir,
                                      qconf->frag.pool_indir);
    else
        rc = rte_ipv6_fragment_packet(m, tbl->m_table + len, n, mtu_size, qconf->frag.pool_dir,
                                      qconf->frag.pool_indir);
...
static inline int32_t send_single_packet(struct rte_mbuf *m, uint16_t port, uint8_t proto)
{
    uint32_t lcore_id;
    uint16_t len;
...
static inline void inbound_sp_sa(struct sp_ctx *sp, struct sa_ctx *sa, struct traffic_type *ip,
                                 uint16_t lim, struct ipsec_spd_stats *stats)
{
    ...
    rte_acl_classify((struct rte_acl_ctx *)sp, ip->data, ip->res, ip->num, DEFAULT_MAX_CATEGORIES);
...
static void split46_traffic(struct ipsec_traffic *trf, struct rte_mbuf *mb[], uint32_t num)
{
    uint32_t i, n4, n6;
    struct ip *ip;
    struct rte_mbuf *m;
    ...
    for (i = 0; i < num; i++) {
        ...
        if (ip->ip_v == IPVERSION) {
            trf->ip4.pkts[n4] = m;
            trf->ip4.data[n4] = rte_pktmbuf_mtod_offset(m, uint8_t *, offsetof(struct ip, ip_p));
            n4++;
        } else if (ip->ip_v == IP6_VERSION) {
            trf->ip6.pkts[n6] = m;
            trf->ip6.data[n6] =
                    rte_pktmbuf_mtod_offset(m, uint8_t *, offsetof(struct ip6_hdr, ip6_nxt));
            n6++;
        } else
            free_pkts(&m, 1);
    }
...
static inline void process_pkts_inbound(struct ipsec_ctx *ipsec_ctx, struct ipsec_traffic *traffic)
{
    ...
    if (app_sa_prm.enable == 0) {
        nb_pkts_in =
                ipsec_inbound(ipsec_ctx, traffic->ipsec.pkts, traffic->ipsec.num, MAX_PKT_BURST);
        split46_traffic(traffic, traffic->ipsec.pkts, nb_pkts_in);
    } else {
        inbound_sa_lookup(ipsec_ctx->sa_ctx, traffic->ipsec.pkts, traffic->ipsec.saptr,
                          traffic->ipsec.num);
        ipsec_process(ipsec_ctx, traffic);
    }

    inbound_sp_sa(ipsec_ctx->sp4_ctx, ipsec_ctx->sa_ctx, &traffic->ip4, n_ip4,
                  &core_statistics[lcoreid].inbound.spd4);

    inbound_sp_sa(ipsec_ctx->sp6_ctx, ipsec_ctx->sa_ctx, &traffic->ip6, n_ip6,
                  &core_statistics[lcoreid].inbound.spd6);
}

static inline void outbound_spd_lookup(struct sp_ctx *sp, struct traffic_type *ip,
                                       struct traffic_type *ipsec, struct ipsec_spd_stats *stats)
{
    struct rte_mbuf *m;
    uint32_t i, j, sa_idx;
...
static inline void process_pkts_outbound(struct ipsec_ctx *ipsec_ctx, struct ipsec_traffic *traffic)
{
    struct rte_mbuf *m;
    ...
    outbound_spd_lookup(ipsec_ctx->sp4_ctx, &traffic->ip4, &traffic->ipsec,
                        &core_statistics[lcoreid].outbound.spd4);

    outbound_spd_lookup(ipsec_ctx->sp6_ctx, &traffic->ip6, &traffic->ipsec,
                        &core_statistics[lcoreid].outbound.spd6);

    if (app_sa_prm.enable == 0) {
        nb_pkts_out = ipsec_outbound(ipsec_ctx, traffic->ipsec.pkts, traffic->ipsec.res,
        ...
        outbound_sa_lookup(ipsec_ctx->sa_ctx, traffic->ipsec.res, traffic->ipsec.saptr,
                           traffic->ipsec.num);
        ipsec_process(ipsec_ctx, traffic);
    }
}
...
static inline int32_t get_hop_for_offload_pkt(struct rte_mbuf *pkt, int is_ipv6)
{
    struct ipsec_mbuf_metadata *priv;
    struct ipsec_sa *sa;
...
static inline void route4_pkts(struct rt_ctx *rt_ctx, struct rte_mbuf *pkts[], uint8_t nb_pkts)
{
...
static inline void route6_pkts(struct rt_ctx *rt_ctx, struct rte_mbuf *pkts[], uint8_t nb_pkts)
{
...
static void drain_inbound_crypto_queues(const struct lcore_conf *qconf, struct ipsec_ctx *ctx)
{
    uint32_t n;
    ...
    if (app_sa_prm.enable == 0) {
        ...
        n = ipsec_inbound_cqp_dequeue(ctx, trf.ipsec.pkts, RTE_DIM(trf.ipsec.pkts));
...
static void drain_outbound_crypto_queues(const struct lcore_conf *qconf, struct ipsec_ctx *ctx)
{
    uint32_t n;
    struct ipsec_traffic trf;
    ...
    if (app_sa_prm.enable == 0) {
        /* dequeue packets from crypto-queue */
        n = ipsec_outbound_cqp_dequeue(ctx, trf.ipsec.pkts, RTE_DIM(trf.ipsec.pkts));
...
void ipsec_poll_mode_worker(void)
{
    ...
    qconf->inbound.session_priv_pool = socket_ctx[socket_id].session_priv_pool;
    qconf->outbound.sp4_ctx = socket_ctx[socket_id].sp_ip4_out;
    qconf->outbound.sp6_ctx = socket_ctx[socket_id].sp_ip6_out;
    qconf->outbound.sa_ctx = socket_ctx[socket_id].sa_out;
    qconf->outbound.cdev_map = cdev_map_out;
    qconf->outbound.session_pool = socket_ctx[socket_id].session_pool;
    qconf->outbound.session_priv_pool = socket_ctx[socket_id].session_priv_pool;
    ...
    if (rc != 0) {
        RTE_LOG(ERR, IPSEC, "SAD cache init on lcore %u, failed with code: %d\n", lcore_id, rc);
        return;
    }
    ...
    for (i = 0; i < qconf->nb_rx_queue; i++) {
        ...
        RTE_LOG(INFO, IPSEC, " -- lcoreid=%u portid=%u rxqueueid=%hhu\n", lcore_id, portid,
                queueid);
    }
...
static uint8_t get_port_nb_rx_queues(const uint16_t port)
{
    int32_t queue = -1;
    uint16_t i;
...
static int32_t init_lcore_rx_queues(void)
{
...
static int64_t parse_decimal(const char *str)
{
    ...
    if ((str[0] == '\0') || (end == NULL) || (*end != '\0') || num > INT64_MAX)
        return -1;
...
static int32_t add_mapping(struct rte_hash *map, const char *str, uint16_t cdev_id, uint16_t qp,
                           struct lcore_params *params, struct ipsec_ctx *ipsec_ctx,
...
static int32_t add_cdev_mapping(struct rte_cryptodev_info *dev_info, uint16_t cdev_id, uint16_t qp,
                                struct lcore_params *params)
{
    ...
    for (i = dev_info->capabilities; i->op != RTE_CRYPTO_OP_TYPE_UNDEFINED; i++) {
        ...
        if (i->sym.xform_type == RTE_CRYPTO_SYM_XFORM_AEAD) {
            ret |= add_mapping(map, str, cdev_id, qp, params, ipsec_ctx, NULL, NULL, i);
            continue;
        }
        ...
        for (j = dev_info->capabilities; j->op != RTE_CRYPTO_OP_TYPE_UNDEFINED; j++) {
            ...
            ret |= add_mapping(map, str, cdev_id, qp, params, ipsec_ctx, i, j, NULL);
        }
    }
...
static int check_cryptodev_mask(uint8_t cdev_id)
{
...
static uint16_t cryptodevs_init(uint16_t req_queue_num)
{
    ...
    const uint64_t mseg_flag = multi_seg_required() ? RTE_CRYPTODEV_FF_IN_PLACE_SGL : 0;
    ...
    for (cdev_id = 0; cdev_id < rte_cryptodev_count(); cdev_id++) {
        ...
        if ((mseg_flag & cdev_info.feature_flags) != mseg_flag)
            rte_exit(EXIT_FAILURE, "Device %hd does not support \'%s\' feature\n", cdev_id,
        ...
        while (qp < max_nb_qps && i < nb_lcore_params) {
            if (add_cdev_mapping(&cdev_info, cdev_id, qp, &lcore_params[idx]))
                qp++;
            idx++;
        ...
        if (dev_max_sess != 0 && dev_max_sess < get_nb_crypto_sessions())
        ...
        qp_conf.mp_session = socket_ctx[dev_conf.socket_id].session_pool;
        qp_conf.mp_session_private = socket_ctx[dev_conf.socket_id].session_priv_pool;
...
static void port_init(uint16_t portid, uint64_t req_rx_offloads, uint64_t req_tx_offloads)
{
    ...
    if (ret != 0)
        rte_exit(EXIT_FAILURE, "Error getting MAC address (port %u): %s\n", portid,
                 rte_strerror(-ret));
    ...
    printf("port %u configuring rx_offloads=0x%" PRIx64 ", tx_offloads=0x%" PRIx64 "\n", portid,
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        ...
        ret = rte_eth_tx_queue_setup(portid, tx_queueid, nb_txd, socket_id, txconf);
        if (ret < 0)
            rte_exit(EXIT_FAILURE,
        ...
        for (queue = 0; queue < qconf->nb_rx_queue; ++queue) {
            ...
            printf("Setup rxq=%d,%d,%d\n", portid, rx_queueid, socket_id);

            rxq_conf = dev_info.default_rxconf;
            rxq_conf.offloads = local_port_conf.rxmode.offloads;
            ret = rte_eth_rx_queue_setup(portid, rx_queueid, nb_rxd, socket_id, &rxq_conf,
...
static void session_pool_init(struct socket_ctx *ctx, int32_t socket_id, size_t sess_sz)
{
    ...
    nb_sess = (get_nb_crypto_sessions() + CDEV_MP_CACHE_SZ * rte_lcore_count());
    nb_sess = RTE_MAX(nb_sess, CDEV_MP_CACHE_SZ * CDEV_MP_CACHE_MULTIPLIER);
    sess_mp = rte_cryptodev_sym_session_pool_create(mp_name, nb_sess, sess_sz, CDEV_MP_CACHE_SZ, 0,
                                                    socket_id);
    ctx->session_pool = sess_mp;

    if (ctx->session_pool == NULL)
        rte_exit(EXIT_FAILURE, "Cannot init session pool on socket %d\n", socket_id);
    else
        printf("Allocated session pool on socket %d\n", socket_id);
}

static void session_priv_pool_init(struct socket_ctx *ctx, int32_t socket_id, size_t sess_sz)
{
    char mp_name[RTE_MEMPOOL_NAMESIZE];
    struct rte_mempool *sess_mp;
    uint32_t nb_sess;
    ...
    nb_sess = (get_nb_crypto_sessions() + CDEV_MP_CACHE_SZ * rte_lcore_count());
    nb_sess = RTE_MAX(nb_sess, CDEV_MP_CACHE_SZ * CDEV_MP_CACHE_MULTIPLIER);
    sess_mp = rte_mempool_create(mp_name, nb_sess, sess_sz, CDEV_MP_CACHE_SZ, 0, NULL, NULL, NULL,
                                 NULL, socket_id, 0);
    ctx->session_priv_pool = sess_mp;

    if (ctx->session_priv_pool == NULL)
        rte_exit(EXIT_FAILURE, "Cannot init session priv pool on socket %d\n", socket_id);
    else
        printf("Allocated session priv pool on socket %d\n", socket_id);
}

static void pool_init(struct socket_ctx *ctx, int32_t socket_id, uint32_t nb_mbuf)
{
    char s[64];
    int32_t ms;

    snprintf(s, sizeof(s), "mbuf_pool_%d", socket_id);
    ctx->mbuf_pool = rte_pktmbuf_pool_create(s, nb_mbuf, MEMPOOL_CACHE_SIZE, ipsec_metadata_size(),
   ...
   */
    ...
    if (ms != 0) {
        ...
        ctx->mbuf_pool_indir =
                rte_pktmbuf_pool_create(s, nb_mbuf, MEMPOOL_CACHE_SIZE, 0, 0, socket_id);
    }
...
static inline int inline_ipsec_event_esn_overflow(struct rte_security_ctx *ctx, uint64_t md)
{
    struct ipsec_sa *sa;
...
static int inline_ipsec_event_callback(uint16_t port_id, enum rte_eth_event_type type, void *param,
                                       void *ret_param)
{
    uint64_t md;
    struct rte_eth_event_ipsec_desc *event_desc = NULL;
    struct rte_security_ctx *ctx = (struct rte_security_ctx *)rte_eth_dev_get_sec_ctx(port_id);
...
static int ethdev_reset_event_callback(uint16_t port_id, enum rte_eth_event_type type,
...
static uint16_t rx_callback(__rte_unused uint16_t port, __rte_unused uint16_t queue,
    ...
    for (i = 0; i != nb_pkts; i++) {
        ...
        if (eth->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
            ...
            if (rte_ipv4_frag_pkt_is_fragmented(iph)) {
                ...
                mb = rte_ipv4_frag_reassemble_packet(lc->frag.tbl, &lc->frag.dr, mb, tm, iph);
            ...
            if (fh != NULL) {
                mb->l2_len = sizeof(*eth);
                mb->l3_len = (uintptr_t)fh - (uintptr_t)iph + sizeof(*fh);
                tm = (tm != 0) ? tm : rte_rdtsc();
                mb = rte_ipv6_frag_reassemble_packet(lc->frag.tbl, &lc->frag.dr, mb, tm, iph, fh);
                if (mb != NULL)
...
static int reassemble_lcore_init(struct lcore_conf *lc, uint32_t cid)
{
    ...
    frag_cycles = (rte_get_tsc_hz() + NS_PER_S - 1) / NS_PER_S * frag_ttl_ns;

    lc->frag.tbl = rte_ip_frag_table_create(frag_tbl_sz, FRAG_TBL_BUCKET_ENTRIES, frag_tbl_sz,
                                            frag_cycles, sid);
    if (lc->frag.tbl == NULL) {
...
static int reassemble_init(void)
{
    int32_t rc;
    uint32_t i, lc;
...
static void create_default_ipsec_flow(uint16_t port_id, uint64_t rx_offloads)
{
...
static void inline_sessions_free(struct sa_ctx *sa_ctx)
{
    ...
    for (i = 0; i < sa_ctx->nb_sa; i++) {
        ...
        ret = rte_security_session_destroy(rte_eth_dev_get_sec_ctx(sa->portid), ips->security.ses);
        if (ret)
            RTE_LOG(ERR, IPSEC,
                    "Failed to destroy security "
                    "session type %d, spi %d\n",
                    ips->type, sa->spi);
    }
}

static uint32_t calculate_nb_mbufs(uint16_t nb_ports, uint16_t nb_crypto_qp, uint32_t nb_rxq,
                                   uint32_t nb_txq)
{
    return RTE_MAX((nb_rxq * nb_rxd + nb_ports * nb_lcores * MAX_PKT_BURST +
                    ...
                    nb_lcores * frag_tbl_sz * FRAG_TBL_BUCKET_ENTRIES),
                   8192U);
}

static int handle_telemetry_cmd_ipsec_secgw_stats(const char *cmd __rte_unused, const char *params,
                                                  struct rte_tel_data *data)
{
    ...
    rte_tel_data_add_dict_u64(data, "packets received", total_pkts_rx);

    rte_tel_data_add_dict_u64(data, "packets transmitted", total_pkts_tx);

    rte_tel_data_add_dict_u64(data, "packets dropped", total_pkts_dropped);

    return 0;
}

static void update_lcore_statistics(struct ipsec_core_statistics *total, uint32_t coreid)
{
...
static void update_statistics(struct ipsec_core_statistics *total, uint32_t coreid)
{
...
static int handle_telemetry_cmd_ipsec_secgw_stats_outbound(const char *cmd __rte_unused,
                                                           const char *params,
                                                           struct rte_tel_data *data)
{
    ...
    rte_tel_data_add_dict_u64(spd4_data, "protect", total_stats.outbound.spd4.protect);
    rte_tel_data_add_dict_u64(spd4_data, "bypass", total_stats.outbound.spd4.bypass);
    rte_tel_data_add_dict_u64(spd4_data, "discard", total_stats.outbound.spd4.discard);

    rte_tel_data_add_dict_container(data, "spd4", spd4_data, 0);

    /* add spd 6 telemetry key/values pairs */

    rte_tel_data_add_dict_u64(spd6_data, "protect", total_stats.outbound.spd6.protect);
    rte_tel_data_add_dict_u64(spd6_data, "bypass", total_stats.outbound.spd6.bypass);
    rte_tel_data_add_dict_u64(spd6_data, "discard", total_stats.outbound.spd6.discard);

    rte_tel_data_add_dict_container(data, "spd6", spd6_data, 0);

    /* add sad telemetry key/values pairs */

    rte_tel_data_add_dict_u64(sad_data, "miss", total_stats.outbound.sad.miss);
...
static int handle_telemetry_cmd_ipsec_secgw_stats_inbound(const char *cmd __rte_unused,
                                                          const char *params,
                                                          struct rte_tel_data *data)
{
    struct ipsec_core_statistics total_stats;
    ...
    rte_tel_data_add_dict_u64(sad_data, "miss", total_stats.inbound.sad.miss);

    rte_tel_data_add_dict_container(data, "sad", sad_data, 0);

    /* add spd 4 telemetry key/values pairs */

    rte_tel_data_add_dict_u64(spd4_data, "protect", total_stats.inbound.spd4.protect);
    rte_tel_data_add_dict_u64(spd4_data, "bypass", total_stats.inbound.spd4.bypass);
    rte_tel_data_add_dict_u64(spd4_data, "discard", total_stats.inbound.spd4.discard);

    rte_tel_data_add_dict_container(data, "spd4", spd4_data, 0);

    /* add spd 6 telemetry key/values pairs */

    rte_tel_data_add_dict_u64(spd6_data, "protect", total_stats.inbound.spd6.protect);
    rte_tel_data_add_dict_u64(spd6_data, "bypass", total_stats.inbound.spd6.bypass);
    rte_tel_data_add_dict_u64(spd6_data, "discard", total_stats.inbound.spd6.discard);
...
static int handle_telemetry_cmd_ipsec_secgw_stats_routing(const char *cmd __rte_unused,
                                                          const char *params,
                                                          struct rte_tel_data *data)
{
    struct ipsec_core_statistics total_stats;
    ...
    rte_tel_data_add_dict_u64(lpm4_data, "miss", total_stats.lpm4.miss);

    rte_tel_data_add_dict_container(data, "IPv4 LPM", lpm4_data, 0);

    /* add lpm 6 telemetry key/values pairs */
    rte_tel_data_add_dict_u64(lpm6_data, "miss", total_stats.lpm6.miss);
...
static void ipsec_secgw_telemetry_init(void)
{
...
int32_t main(int32_t argc, char **argv)
{
    ...
    if ((unprotected_port_mask & enabled_port_mask) != unprotected_port_mask)
   ...
   */
    ...
    if (nb_bufs_in_pool == 0) {
        ...
        nb_bufs_in_pool = calculate_nb_mbufs(nb_ports, nb_crypto_qp, nb_rxq, nb_txq);
    }
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        ...
        session_priv_pool_init(&socket_ctx[socket_id], socket_id, sess_sz);
    }
   ...
   */
     ...
     */
        ...
        rte_eth_dev_callback_register(portid, RTE_ETH_EVENT_INTR_RESET, ethdev_reset_event_callback,
                                      NULL);

        rte_eth_dev_callback_register(portid, RTE_ETH_EVENT_IPSEC, inline_ipsec_event_callback,
                                      NULL);
    }
    ...
    for (i = 0; i < NB_SOCKETS && i < rte_socket_count(); i++) {
        ...
        if ((socket_ctx[socket_id].mbuf_pool != NULL) && (socket_ctx[socket_id].sa_in == NULL) &&
    ...
    RTE_ETH_FOREACH_DEV(portid)
    {
        ...
        if (flow_info_tbl[portid].rx_def_flow) {
            ...
            ret = rte_flow_destroy(portid, flow_info_tbl[portid].rx_def_flow, &err);
            if (ret)
                RTE_LOG(ERR, IPSEC,
        ...
        if (ret != 0)
            RTE_LOG(ERR, IPSEC, "rte_eth_dev_stop: err=%s, port=%u\n", rte_strerror(-ret), portid);
